...
```{r load packages}
needed.packages <- c("tidyverse", "here", "functional", "gplots", "dplyr", "GeneOverlap", "R.utils", "reshape2","magrittr","data.table", "RColorBrewer","preprocessCore", "ARTool","emmeans", "phia", "gprofiler2","rlist", "plotly","kableExtra","foreach","doParallel","doFuture","pheatmap","Hmisc")
...
for(i in 1:length(modules.tissue)){
  for(j in 1:length(modules.tissue)){
    ...
    #pvalue[[i]] <- rcorr(modules.tissue[[i]], modules.tissue[[j]])$P
    #names.correlations[[i]] <- paste(names(modules.tissue[i]), "vs", names(modules.tissue[j]))
    pheatmap(correlations, show_rownames = T,show_colnames = T, fontsize_col = 8, fontsize_row = 8, color = colorRampPalette(rev(brewer.pal(n = 12, name =
  "Paired")))(100))
    cat("\n \n")
  }
}
#names.correlations <- unlist(names.correlations)
#names(correlations) <- names.correlations
#names(pvalue) <- names.correlations
```
...
```{r corr loop liver, results='asis'}
  for(j in 1:length(modules.tissue)){
    ...
    pheatmap(correlations, show_rownames = T,show_colnames = T, fontsize_col = 4, fontsize_row = 4, color = colorRampPalette(rev(brewer.pal(n = 12, name =
  "Paired")))(100), cluster_rows = F)
    cat("\n \n")
  }
