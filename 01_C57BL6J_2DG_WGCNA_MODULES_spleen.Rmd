---
title: "Module Generation Spleen"
author: Ann Wells
...
threshold <- softthreshold(log.tdata.FPKM.sample.info.subset.spleen.WGCNA)
```
...
soft.power <- which(threshold$fitIndices$SFT.R.sq >= 0.87)[1]
cat("\n", "Power chosen") 
soft.power
cat("\n \n")
...
```{r}
me.list <- moduleEigengenes(log.tdata.FPKM.sample.info.subset.spleen.WGCNA, colors=cols) 
...
merge <- mergeCloseModules(log.tdata.FPKM.sample.info.subset.spleen.WGCNA, cols, cutHeight=merge.thresh, verbose=0)
