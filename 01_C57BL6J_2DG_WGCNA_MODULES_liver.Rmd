...
soft <- pickSoftThreshold(log.tdata.FPKM.sample.info.subset.liver.WGCNA, powerVector=powers, verbose=0)
```
...
```{r}
me.list <- moduleEigengenes(log.tdata.FPKM.sample.info.subset.liver.WGCNA, colors=cols) 
...
merge <- mergeCloseModules(log.tdata.FPKM.sample.info.subset.liver.WGCNA, cols, cutHeight=merge.thresh, verbose=0)
