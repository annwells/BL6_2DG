---
title: "Chang_B6_96hr_4wk 2DG Pathway Analysis Skeletal Muscle, Small Intestine, and Spleen 4wks"
author: Ann Wells
...
WGCNA.gene <-readRDS(here("Data","SM.SI.spleen.4wks","Chang_B6_96hr_4wk_gprofiler_gene_annotation_list_SM_SI_spleen_4wks_WGCNA.RData"))
...
log.tdata.FPKM.sample.info.subset.SM.SI.spleen.4wks <- log.tdata.FPKM.sample.info.subset %>% rownames_to_column() %>% filter(Tissue %in% c("Skeletal Muscle","Small Intestine","Spleen")) %>% filter(Time == "4 wks") %>% column_to_rownames()
...
```{r}
log.tdata.FPKM.sample.info.subset.SM.SI.spleen.4wks$Time <- as.factor(log.tdata.FPKM.sample.info.subset.SM.SI.spleen.4wks$Time)
...
stat.table <- foreach(i = 1:length(genes),.packages = c("ARTool","tidyverse","kableExtra")) %dopar% {
   m2 <- art(data = log.tdata.FPKM.sample.info.subset.SM.SI.spleen.4wks, log.tdata.FPKM.sample.info.subset.SM.SI.spleen.4wks[,genes[i]] ~ Tissue*Treatment)
...
saveRDS(stat.table,here("Data","SM.SI.spleen.4wks","Chang_B6_96hr_4wk_WGCNA_gene_ANOVA_ART_SM_SI_spleen_4wk.RData"))
```

```{r, results='asis', cache = TRUE}

stat.table <- readRDS(here("Data","SM.SI.spleen.4wks","Chang_B6_96hr_4wk_WGCNA_gene_ANOVA_ART_SM_SI_spleen_4wk.RData"))
