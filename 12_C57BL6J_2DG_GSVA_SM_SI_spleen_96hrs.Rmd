---
title: "Chang B6 96hr 4wk 2DG Gene Set Variation Analysis Skeletal Muscle, Small Intestine, and Spleen 96hrs"
author: Ann Wells
...
log.tdata.FPKM.sample.info.subset.SM.SI.spleen.96hrs <- log.tdata.FPKM.sample.info.subset %>% 
  rownames_to_column() %>% 
...
log.data.FPKM.subset.SM.SI.spleen.96hrs <- log.tdata.FPKM.sample.info.subset.SM.SI.spleen.96hrs %>%
...
tES <- GSVA.modules(modules, log.data.FPKM.subset.SM.SI.spleen.96hrs)
```
...
for(j in 1:length(tES)){
  ...
  m <- art(data = log.tdata.FPKM.sample.info.subset.SM.SI.spleen.96hrs, tES[[j]][,i] ~ Time*Treatment*Tissue)
...
plot(Treat.diff, comparisons = T, colors = "red")
```
#### Treatment bar plot
```{r}
p <- ggplot()
...
Tissue.pairs <- pairs(Tissue.diff, adjust = "Dunn")
panderOptions('knitr.auto.asis', FALSE)
knitr::kable(Tissue.pairs$emmeans)
knitr::kable(Tissue.pairs$contrasts)
plot(Tissue.diff, adjust = "Dunn", comparisons = T, colors = "red")
```
