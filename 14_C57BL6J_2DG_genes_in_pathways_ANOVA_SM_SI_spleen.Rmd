---
title: "Chang_B6_96hr_4wk 2DG Pathway Analysis SM.SI.spleen, Small Intestine, and Spleen"
author: Ann Wells
...
WGCNA.gene <-readRDS(here("Data","SM.SI.spleen","Chang_B6_96hr_4wk_gprofiler_gene_annotation_list_SM_SI_spleen_WGCNA.RData"))
...
log.tdata.FPKM.sample.info <- cbind(log.tdata.FPKM, tdata.FPKM.sample.info[,27238:27240])  log.tdata.FPKM.sample.info <- log.tdata.FPKM.sample.info %>% rownames_to_column() %>% filter(rowname != "A113") %>% column_to_rownames()

log.tdata.FPKM.subset <- log.tdata.FPKM[,colMeans(log.tdata.FPKM != 0) > 0.5] 

log.tdata.FPKM.sample.info.subset <- cbind(log.tdata.FPKM.subset,tdata.FPKM.sample.info[,27238:27240])  log.tdata.FPKM.sample.info <- log.tdata.FPKM.sample.info %>% rownames_to_column() %>% filter(rowname != "A113") %>% column_to_rownames()

log.tdata.FPKM.sample.info.subset.SM.SI.spleen <- log.tdata.FPKM.sample.info.subset %>% rownames_to_column() %>% filter(Tissue == "SM.SI.spleen") %>% column_to_rownames()
...
stat.table <- foreach(i = 1:length(genes),.packages = c("ARTool","tidyverse","kableExtra")) %dopar% {
   m2 <- art(data = log.tdata.FPKM.sample.info.subset.SM.SI.spleen, log.tdata.FPKM.sample.info.subset.SM.SI.spleen[,genes[i]] ~ Time*Treatment*Tissue)
...
saveRDS(stat.table,here("Data","SM.SI.spleen","Chang_B6_96hr_4wk_WGCNA_gene_ANOVA_ART.RData"))
```

```{r, results='asis', cache = TRUE}

stat.table <- readRDS(here("Data","SM.SI.spleen","Chang_B6_96hr_4wk_WGCNA_gene_ANOVA_ART.RData"))
