...
for(m in module.labels) {
    ...
    cat("\n###",name, "{.tabset .tabset-fade .tabset-pills}","\n")
          for(i in 1:10){
             cat("\n####",hub.met$gene[i],"\n")
             m <- art(data = log.tdata.FPKM.sample.info.subset.hip.hyp.cortex, log.tdata.FPKM.sample.info.subset.hip.hyp.cortex[,hub.met$Gene[i]] ~ Time*Treatment*Tissue)
             model <- anova(m)
