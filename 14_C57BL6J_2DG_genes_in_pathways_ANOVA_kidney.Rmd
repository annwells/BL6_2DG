---
title: "Chang_B6_96hr_4wk 2DG Pathway Analysis Kidney"
author: Ann Wells
...
WGCNA.gene <-readRDS(here("Data","Kidney","Chang_B6_96hr_4wk_gprofiler_gene_annotation_list_kidney_WGCNA.RData"))
...
log.tdata.FPKM.sample.info.subset.hypothalamus <- log.tdata.FPKM.sample.info.subset %>% rownames_to_column() %>% filter(Tissue == "Kidney") %>% column_to_rownames()
...
stat.table <- foreach(i = 1:length(genes),.packages = c("ARTool","tidyverse","kableExtra")) %dopar% {
   m2 <- art(data = log.tdata.FPKM.sample.info.subset, log.tdata.FPKM.sample.info.subset[,genes[i]] ~ Time*Treatment)
...
saveRDS(stat.table,here("Data","BXSB_WGCNA_gene_ANOVA_ART.RData"))
```

```{r, results='asis', cache = TRUE}

stat.table <- readRDS(here("Data","BXSB_WGCNA_gene_ANOVA_ART.RData"))
