---
title: "Module Generation Pre-frontal Cortex"
author: Ann Wells
...
threshold <- softthreshold(log.tdata.FPKM.sample.info.subset.prefrontal.cortex.WGCNA)
```
...
```{r}
soft.power <- which(threshold$fitIndices$SFT.R.sq >= 0.8)[1]
cat("\n", "Power chosen") 
soft.power
cat("\n \n")
...
```{r}
me.list <- moduleEigengenes(log.tdata.FPKM.sample.info.subset.prefrontal.cortex.WGCNA, colors=cols) 
...
merge <- mergeCloseModules(log.tdata.FPKM.sample.info.subset.prefrontal.cortex.WGCNA, cols, cutHeight=merge.thresh, verbose=0)
...
module.membership <- data.frame(
  Gene=colnames(log.tdata.FPKM.sample.info.subset.prefrontal.cortex.WGCNA),
