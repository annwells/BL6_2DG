---
title: "Chang_B6_96hr_4wk 2DG Pathway Analysis Prefrontal Cortex"
author: Ann Wells
...
WGCNA.gene <-readRDS(here("Data","Prefrontal Cortex","Chang_B6_96hr_4wk_gprofiler_gene_annotation_list_prefrontal_cortex_WGCNA.RData"))
...
```{r}
log.tdata.FPKM.sample.info.subset.prefrontal.cortex$Time <- as.factor(log.tdata.FPKM.sample.info.subset.prefrontal.cortex$Time)
log.tdata.FPKM.sample.info.subset.prefrontal.cortex$Treatment <- as.factor(log.tdata.FPKM.sample.info.subset.prefrontal.cortex$Treatment)
log.tdata.FPKM.sample.info.subset.prefrontal.cortex$Tissue <- as.factor(log.tdata.FPKM.sample.info.subset.prefrontal.cortex$Tissue)
...
stat.table <- foreach(i = 1:length(genes),.packages = c("ARTool","tidyverse","kableExtra")) %dopar% {
   m2 <- art(data = log.tdata.FPKM.sample.info.subset.prefrontal.cortex, log.tdata.FPKM.sample.info.subset.prefrontal.cortex[,genes[i]] ~ Time*Treatment)
...
saveRDS(stat.table,here("Data","Prefrontal Cortex","Chang_B6_96hr_4wk_WGCNA_gene_ANOVA_ART_prefrontal_cortex.RData"))
```

```{r, results='asis', cache = TRUE}

stat.table <- readRDS(here("Data","Prefrontal Cortex","Chang_B6_96hr_4wk_WGCNA_gene_ANOVA_ART_prefrontal_cortex.RData"))
