---
title: "Chang_B6_96hr_4wk 2DG Pathway Analysis Skeletal Muscle"
author: Ann Wells
...
log.tdata.FPKM.sample.info.subset.muscle <- log.tdata.FPKM.sample.info.subset %>% rownames_to_column() %>% filter(Tissue == "Skeletal Muscle") %>% column_to_rownames()
...
```{r}
log.tdata.FPKM.sample.info.subset.muscle$Time <- as.factor(log.tdata.FPKM.sample.info.subset.muscle$Time)
log.tdata.FPKM.sample.info.subset.muscle$Treatment <- as.factor(log.tdata.FPKM.sample.info.subset.muscle$Treatment)
log.tdata.FPKM.sample.info.subset.muscle$Tissue <- as.factor(log.tdata.FPKM.sample.info.subset.muscle$Tissue)
...
stat.table <- foreach(i = 1:length(genes),.packages = c("ARTool","tidyverse","kableExtra")) %dopar% {
   m2 <- art(data = log.tdata.FPKM.sample.info.subset.muscle, log.tdata.FPKM.sample.info.subset.muscle[,genes[i]] ~ Time*Treatment)
...
saveRDS(stat.table,here("Data","Skeletal Muscle","Chang_B6_96hr_4wk_WGCNA_gene_ANOVA_ART_muscle.RData"))
```

```{r, results='asis', cache = TRUE}

stat.table <- readRDS(here("Data","Skeletal Muscle","Chang_B6_96hr_4wk_WGCNA_gene_ANOVA_ART_muscle.RData"))
