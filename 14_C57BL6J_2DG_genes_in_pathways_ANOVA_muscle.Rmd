...
tdata.FPKM.sample.info <- as.data.frame(tdata.FPKM.sample.info) %>% rownames_to_column() %>% filter(rowname != "A113") %>% column_to_rownames()

tdata.FPKM <- readRDS(here("Data","20190406_RNAseq_B6_4wk_2DG_counts_numeric.RData")) 

tdata.FPKM <- as.data.frame(tdata.FPKM) %>% rownames_to_column() %>% filter(rowname != "A113") %>% column_to_rownames()

log.tdata.FPKM <- log(tdata.FPKM + 1)
log.tdata.FPKM <- as.data.frame(log.tdata.FPKM) %>% rownames_to_column() %>% filter(rowname != "A113") %>% column_to_rownames()
