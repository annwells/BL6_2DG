...
tES <- GSVA.modules(modules, log.data.FPKM.subset)
```
...
for(j in 1:length(tES)){
  ...
  m <- art(data = log.tdata.FPKM.sample.info.subset, tES[[j]][,i] ~ Time*Treatment*Tissue)
...
## Post-hoc Analysis {.tabset .tabset-fade .tabset-pills}

### Treatment {.tabset .tabset-fade .tabset-pills}
#### Contrast
```{r}
Treat.diff <- emmeans(artlm(m,"Treatment"), pairwise ~ Treatment)
panderOptions('knitr.auto.asis', FALSE)
knitr::kable(Treat.diff$emmeans)
knitr::kable(Treat.diff$contrasts)
```

#### Bar plot
```{r}
bar.data <- as.data.frame(cbind(tES,Time = log.tdata.FPKM.sample.info.subset$Time, Treatment = log.tdata.FPKM.sample.info.subset$Treatment,Tissue = log.tdata.FPKM.sample.info.subset$Tissue))
p <- ggplot(bar.data,aes(x = Treatment, y = V1, fill = Treatment)) + geom_bar(stat = "identity") +
   geom_errorbar()
```
...
### Time by treatment {.tabset .tabset-fade .tabset-pills}
#### Contrast
```{r, results='asis'}
TI <- testInteractions(artlm(m,"Time:Treatment"), pairwise = c("Time", 'Treatment'), adjustment = "BH")
panderOptions('knitr.auto.asis', FALSE)
print(pander(TI))
```
...
#### Contrast
```{r, results='asis'}
TI <- testInteractions(artlm(m,"Time:Tissue"), pairwise = c("Time", 'Tissue'), adjustment = "BH")
panderOptions('knitr.auto.asis', FALSE)
print(pander(TI))
```
...
#### Contrast
```{r, results='asis'}
TI <- testInteractions(artlm(m,"Treatment:Tissue"), pairwise = c('Treatment', 'Tissue'), adjustment = "BH")
panderOptions('knitr.auto.asis', FALSE)
print(pander(TI))
```
