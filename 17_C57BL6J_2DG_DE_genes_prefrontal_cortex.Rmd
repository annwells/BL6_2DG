---
title: "Chang_B6_96hr_4wk 2DG Differential Expression Prefrontal Cortex"
author: Ann Wells
...
log.tdata.FPKM.sample.info.subset.cortex <- log.tdata.FPKM.sample.info %>% rownames_to_column() %>% filter(Tissue == "Pre-frontal Cortex") %>% column_to_rownames()

log.tdata.FPKM.subset <- subset(log.tdata.FPKM.sample.info.subset.cortex, select = -c(Time,Treatment,Tissue))
...
```{r ANOVA, cache=TRUE}
log.tdata.FPKM.sample.info.subset.cortex$Time <- as.factor(log.tdata.FPKM.sample.info.subset.cortex$Time)

log.tdata.FPKM.sample.info.subset.cortex$Treatment <- as.factor(log.tdata.FPKM.sample.info.subset.cortex$Treatment)
...
for(i in 1:length(log.tdata.FPKM.subset)){
  m <- art(data = log.tdata.FPKM.sample.info.subset.cortex, log.tdata.FPKM.subset[,i] ~ Time*Treatment)
...
saveRDS(Treatment.sig.only, here("Data","Prefrontal Cortex","Chang_2DG_BL6_DE_genes_prefrontal_cortex.RData"))
```
...
frame %>%
  download_this(
    output_name = "Prefrontal Cortex ANOVA Gene Differential Expression",
